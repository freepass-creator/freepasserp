...
import { ensureRoom, watchProducts, resolveTermForProduct, updateProduct } from "../firebase/firebase-db.js";
...
const INFO_COLS = [
  ...
  { key: 'vehicleStatus', label: '차량상태', align: 'c', filterable: true, w: 85 },
  { key: 'productType',   label: '상품구분', align: 'c', filterable: true, w: 85 },
  { key: 'partnerCode',   label: '공급코드', align: 'c', filterable: true, w: 80 },
  { key: 'carNo',         label: '차량번호', align: 'c', sticky: true, searchable: true, w: 95 },
  { key: 'maker',         label: '제조사',   align: 'c', filterable: true, w: 70 },
  { key: 'model',         label: '모델명',   align: 'c', filterable: true, w: 85 },
  ...
  { key: 'color',         label: '색상',     align: 'c', filterable: true, w: 85 },
  { key: 'year',          label: '연식',     align: 'c', filterable: true, w: 60 },
  { key: 'mileage',       label: '주행거리', num: true,  filterable: true, w: 90 },
  { key: 'vehicleClass',  label: '차종구분', align: 'c', filterable: true, w: 85 },
  { key: 'reviewStatus',  label: '심사기준', align: 'c', filterable: true, w: 85 },
  { key: 'minAge',        label: '최저연령', align: 'c', filterable: true, w: 80 },
];
...
function hideDetailPanel() {
  ...
  if ($list) $list.querySelectorAll('tr.is-selected').forEach(tr => tr.classList.remove('is-selected'));
  _syncTopBar(null);
  requestAnimationFrame(() => { $detailPanel.style.transition = ''; });
}
qs('#closeDetailBtn')?.addEventListener('pointerdown', hideDetailPanel);
...
document.addEventListener('contextmenu', (e) => {
  ...
  if (role === 'agent' || role === 'agent_manager') {
    items += `<button type="button" class="pm-ctx-item" data-action="inquiry">
      ...
      문의하기</button>`;
    items += `<button type="button" class="pm-ctx-item" data-action="contract">
  ...
  menuEl.className = 'pm-ctx-menu';
  menuEl.innerHTML = items;
  document.body.appendChild(menuEl);
  _ctxMenu = menuEl;
  menuEl.style.cssText = `position:fixed;top:${e.clientY}px;left:${e.clientX}px;z-index:9999;`;
  ...
  menuEl.addEventListener('click', async (ev) => {
    ...
    if (action === 'contract') { await handleContract(); }
    if (action === 'share') {
...
function renderMobileCatalogDetail(product, { actionsHtml = '' } = {}) {
  ...
  return galleryHtml
    ...
    + renderCatalogPriceTable(priceRows, { showFee: false, guideNote, clawbackNote: '' })
